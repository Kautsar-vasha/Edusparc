...
<div class="container-fluid py-2 py-md-4">
    ...
    <div class="mb-3 mb-md-4">
        <a href="javascript:history.back()" class="text-decoration-none text-muted mb-2 d-inline-block small">
            <i class="bi bi-arrow-left me-1"></i> Kembali
        </a>
        <h3 class="fw-bold text-dark mb-0 fs-4 fs-md-3">
            <i class="bi bi-person-badge text-primary me-2"></i>Profil & Riwayat Karakter Siswa
        </h3>
    </div>

    <div class="row g-3 g-md-4">
        {{-- Kartu Profil Siswa (Kiri - Menyesuaikan Tablet dan PC) --}}
        <div class="col-12 col-md-5 col-lg-4">
            <div class="glass-card p-3 p-md-4 text-center h-100 d-flex flex-column justify-content-center">
                ...
                <h4 class="fw-bold text-dark mb-1 fs-5 fs-md-4">{{ $student->name }}</h4>
                ...
                <hr class="text-muted w-75 mx-auto my-2">

                <h6 class="text-muted text-uppercase small fw-bold mt-2 mt-md-3">Total Poin Saat Ini</h6>
                <h1 class="display-4 display-md-3 fw-bold {{ $student->total_points >= 0 ? 'text-success' : 'text-danger' }}">
                    {{ $student->total_points > 0 ? '+' : '' }}{{ $student->total_points }}
                </h1>

                @if($student->total_points >= 0)
                    <div class="badge bg-success-subtle text-success p-2 px-3 rounded-pill mt-2 small">
                        <i class="bi bi-shield-check me-1"></i> Perilaku Terjaga Baik
                    </div>
                @else
                    <div class="badge bg-danger-subtle text-danger p-2 px-3 rounded-pill mt-2 small">
        ...
        <div class="col-12 col-md-7 col-lg-8">
            <div class="glass-card p-3 p-md-4 h-100">
                <h5 class="fw-bold mb-3 mb-md-4 fs-6 fs-md-5"><i class="bi bi-clock-history text-primary me-2"></i>Daftar Riwayat Tercatat</h5>

                <!-- 1. Tampilan Desktop & Tablet (Tabel Responsif) -->
                                    ...
                                    @if($v->tanggapan_ortu)
                                        ...
                                        @else
                                            <div class="mt-2 bg-secondary bg-opacity-10 p-2 rounded border-start border-3 border-secondary">
                                                <span class="text-muted" style="font-size: 0.75rem;"><i class="bi bi-lock-fill me-1"></i>Pesan balasan ortu disembunyikan (Khusus Wali/BK)</span>
                                            </div>
                                        @endif
                                    @endif
                                </td>
                ...
                <!-- 2. Tampilan Khusus Mobile / HP (List Vertikal berupa Kartu) -->
                <div class="d-block d-md-none">
                    @forelse($riwayat as $v)
                        <div class="card border shadow-sm mb-3">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <span class="badge bg-light text-secondary border small">{{ $v->category }}</span>
                                    <span class="fw-bold fs-6 {{ $v->jenis_poin == 'positif' ? 'text-success' : 'text-danger' }}">
                                ...
                                @if($v->motivation)
                                    <div class="mt-2 p-2 bg-primary-subtle rounded border-start border-3 border-primary">
                                        <span class="text-primary fw-semibold small"><i class="bi bi-chat-heart-fill me-1"></i>Catatan Guru:</span>
                                        <p class="text-dark fst-italic small mb-0 mt-1">"{{ $v->motivation }}"</p>
                                    </div>
                                @endif

                                {{-- Tanggapan Ortu (Mobile) --}}
                                @if($v->tanggapan_ortu)
                                    @if(isset($is_wali_or_bk) && $is_wali_or_bk)
                                        <div class="mt-2 bg-success bg-opacity-10 p-2 rounded border-start border-3 border-success">
                                            <span class="fw-bold text-success small"><i class="bi bi-reply-all-fill me-1"></i>Balasan Wali Murid:</span>
                                            <p class="text-dark fst-italic small mb-0 mt-1">"{{ $v->tanggapan_ortu }}"</p>
                                        </div>
                                    @else
                                        <div class="mt-2 bg-secondary bg-opacity-10 p-2 rounded border-start border-3 border-secondary">
                                            <span class="text-muted small"><i class="bi bi-lock-fill me-1"></i>Pesan balasan ortu disembunyikan (Khusus Wali/BK)</span>
                                        </div>
                                    @endif
                                @endif
                            </div>
                    ...
                    @empty
                        <div class="text-center py-5 text-muted fst-italic">
                            ...
                            <span class="small">Belum ada catatan riwayat.</span>
                        </div>
                    @endforelse
                </div>
